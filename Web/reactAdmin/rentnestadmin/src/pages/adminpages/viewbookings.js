...
export default function ViewBookings() {
  document.title = "Rentnest | Booking Property";
  ...
  const column = useMemo(
    () => [
      ...
      {
        name: "Days",
        selector: (row) => row.dayCount + " days",
        sortable: true,
        filter: true,
      },
      ...
      //         onClick={() =>
      //           window.sessionStorage.setItem("route", "serverdetails")
      //         }
      //       >
      ...
      //           row.block == 0 ? `btnButtonwarn` : `btnButtongrey`
      //         }`}
      //         onClick={() => {
      ...
      //       >
      //         <FontAwesomeIcon
      //           icon={row.block == 0 ? faUnlock : faLock}
      //           className="fs-5"
      //         />
      //       </button>
      //     </div>
  ...
  const viewserver = useCallback(
    async (page, size = perPage) => {
      ...
      var response = await fetchData(
        `/viewBooking?&keyword=${searchText}`,
        data
      );
      if (response.status === 200) {
        setTimeout(() => {
          ...
          setTotalRows(
            response.data.length == "0" ? "0" : response.data[0]["Count"]
          );
          setLoading(false);
        }, 500);
      } else {
              ...
              <div className="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                ...
                <div className="page-title d-flex flex-column justify-content-center gap-1 me-3 mb-4">
                  ...
                  <ul className="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                    ...
                    <li className="breadcrumb-item text-muted">Booking Property</li>
                    {/* <!--end::Item--> */}
                  </ul>
                </div>
              </div>
